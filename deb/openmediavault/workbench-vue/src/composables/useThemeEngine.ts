...
const injectFontFace = (fonts: ThemeRegistryEntry['assets']['fonts'] = []) => {
...
const applyCssVariables = (entries: Record<string, string> = {}, prefix = '') => {
  Object.entries(entries).forEach(([key, value]) => {
    document.documentElement.style.setProperty(`--omv-${prefix}${key}`, value);
  });
};
